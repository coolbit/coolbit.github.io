...
<link href="{{.HomeURL}}/_pagefind/pagefind-ui.css" rel="stylesheet">
<script src="{{.HomeURL}}/_pagefind/pagefind-ui.js"></script>
</head>
<body>
...
<div id="search" style="margin:2rem 0"></div>
<script>window.addEventListener('DOMContentLoaded',()=>new PagefindUI({element:'#search',showImages:false}))</script>
{{if not .Posts}}<p class="empty">No posts yet.</p>{{end}}
<div class="post-list">
{{range .Posts}}
<article class="post-card">
  {{if .CoverImage}}<a href="{{$.PostsBase}}{{.ID}}.html"><img src="{{.CoverImage}}" class="card-cover" alt=""></a>{{end}}
  <div class="post-meta">
...
.card-cover{width:100%;height:200px;object-fit:cover;border-radius:6px;margin-bottom:1rem;display:block}
...
.cover-img{width:100%;max-height:420px;object-fit:cover;border-radius:8px;margin-bottom:2rem;display:block}
...
.md-body pre{padding:1.25rem;border-radius:8px;overflow-x:auto;margin:1.5rem 0}
.md-body pre code{background:none;padding:0;font-size:.875rem}
